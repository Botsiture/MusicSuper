...
async def skip(cli, message: Message, _, chat_id):
    if not len(message.command) < 2:
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await DevSp.stop_stream(chat_id)
                except:
                    return
        except:
            try:
    ...
    if "live_" in queued:
        ...
        try:
            ...
            image = None
        try:
            await DevSp.skip_stream(chat_id, link, video=status, image=image)
        except:
        ...
        run = await message.reply_photo(
            ...
            caption=_["stream_1"].format(
                f"[https://t.me/](https://t.me/){app.username}?start=info_{videoid}",
                title[:23],
                check[0]["dur"],
                user,
            ),
        ...
        try:
            ...
            image = None
        try:
            await DevSp.skip_stream(chat_id, file_path, video=status, image=image)
        except:
            return await mystic.edit_text(_["call_6"])
        button = stream_markup(_, chat_id)
        img = await gen_thumb(videoid)
        ...
        try:
            await DevSp.skip_stream(chat_id, videoid, video=status)
        except:
            return await message.reply_text(_["call_6"])
        button = stream_markup(_, chat_id)
        run = await message.reply_photo(
        ...
        if videoid == "telegram":
            ...
            try:
                ...
                image = None
        try:
            await DevSp.skip_stream(chat_id, queued, video=status, image=image)
        except:
            return await message.reply_text(_["call_6"])
        if videoid == "telegram":
            button = stream_markup(_, chat_id)
